        ...
        if (sender instanceof Player) {
            ...
            if (player.hasPermission(plugin.getName() + ".reload")) {
                if (strings.length == 0) {
                    ...
                    player.sendMessage(Component.text(Data.PluginReload.text));
                } else
                    player.sendMessage(Component.text(Data.WrongSyntax.text.replace("%syntax%", "/rlplugin")));
            } else
                player.sendMessage(Component.text(Data.NoPerms.text));
        } else {
            if (strings.length == 0) {
                Bukkit.getPluginManager().disablePlugin(plugin);
                Bukkit.getPluginManager().enablePlugin(plugin);
                Bukkit.getConsoleSender().sendMessage(Component.text(Data.PluginReload.text));
            } else
                Bukkit.getConsoleSender().sendMessage(Component.text(Data.WrongSyntax.text.replace("%syntax%", "rlplugin")));
        }
        return true;
    }
}